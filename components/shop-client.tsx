	...
	return (
		<>
			<div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
				<div>
					<p className="text-sm font-semibold uppercase tracking-[0.3em] text-stone-700">Collection</p>
				...
				<div className="flex flex-wrap items-center gap-2 md:justify-end">
					...
					<div className="rounded-full border border-stone-200 bg-white p-1">
						<button type="button" aria-pressed={viewMode === "grid"} onClick={() => setViewMode("grid")} className={`rounded-full px-3 py-1 text-xs font-semibold ${viewMode === "grid" ? "bg-stone-900 text-white" : "text-stone-700"}`}>
							Grid
						</button>
						<button type="button" aria-pressed={viewMode === "list"} onClick={() => setViewMode("list")} className={`rounded-full px-3 py-1 text-xs font-semibold ${viewMode === "list" ? "bg-stone-900 text-white" : "text-stone-700"}`}>
							List
						</button>
					</div>
				</div>
			...
			<div className="mt-8 grid gap-6 lg:grid-cols-[280px_1fr]">
				...
				<Dialog open={isMobileFiltersOpen} onClose={() => setIsMobileFiltersOpen(false)} variant="sheet-bottom" className="h-[80vh] rounded-t-[1.75rem] lg:hidden">
					<div className="flex items-center justify-between border-b border-stone-200 px-5 py-4">
						<div>
							<DialogTitle className="text-sm font-semibold uppercase tracking-[0.2em] text-stone-900">Filters</DialogTitle>
							<p className="mt-1 text-xs text-stone-700">Live updates while you filter</p>
							<p className="mt-2 text-xs font-semibold text-[#1B365D]">{matchingResultsCount} results match your current search and filters</p>
						</div>
						<DialogClose label="Close filters" className="rounded-full border border-stone-200 bg-white p-2 text-stone-700 shadow-sm transition hover:bg-stone-100">
							<svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current stroke-[1.8]">
				...
				<div className="space-y-4">
					{filteredProducts.length === 0 ? (
						<div className="rounded-[1.75rem] border border-dashed border-stone-300 bg-white p-8 text-center shadow-sm">
							...
							<p className="mt-2 text-sm text-stone-700">Try a broader keyword or reset the category filter.</p>
						...
						<div className={viewMode === "grid" ? "grid grid-cols-2 gap-3 md:gap-6 md:grid-cols-3" : "space-y-2"}>
							{visibleProducts.map((product) => (
								<Link key={product.id} href={`/product/${product.slug}`} className={`relative rounded-[1.75rem] bg-white  md:transition md:hover:-translate-y-1 ${viewMode === "list" ? "flex gap-4" : ""}`}>
									<ProductImage src={product.image} alt={product.name} sizes="(min-width: 768px) 33vw, 50vw" className={viewMode === "list" ? "h-32 w-32 shrink-0 rounded-l-[1.75rem]" : "mx-auto aspect-4/5"} />

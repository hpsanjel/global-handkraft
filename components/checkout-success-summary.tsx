...
/**
 * Presentational "thank you" summary shared by the Stripe (/checkout/success)
 * and Vipps (/checkout/vipps/return) confirmation pages, so both providers
 * render an identical order summary without duplicating this JSX. Each page
 * owns its own fetch (from /api/checkout/session or /api/checkout/vipps/session
 * respectively) and passes the normalized OrderSummary shape in as a prop.
 */
export function CheckoutSuccessSummary({ order, isLoading, error }: { order: OrderSummary | null; isLoading: boolean; error: string }) {
	return (
		<>
			{/* Hero */}

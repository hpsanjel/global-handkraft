...
function StarPicker({ value, onChange }: { value: number; onChange: (value: number) => void }) {
	...
	return (
		<div className="flex items-center gap-1" onMouseLeave={() => setHovered(0)}>
			{[1, 2, 3, 4, 5].map((star) => (
				<button key={star} type="button" onClick={() => onChange(star)} onMouseEnter={() => setHovered(star)} aria-label={`${star} star${star > 1 ? "s" : ""}`} className="p-0.5">
...
export function ProductReviews({ productSlug, initialRating, initialReviewCount, initialReviews }: { productSlug: string; initialRating: number; initialReviewCount: number; initialReviews: PublicReview[] }) {
	...
	return (
		<div className="mt-10 rounded-[1.5rem] border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
				...
				<div className="mt-6 space-y-4 border-t border-stone-200 pt-6">
					...
					<div className="grid gap-4 sm:grid-cols-2">
						<label className="space-y-2 text-sm text-stone-600">
							...
							<input value={name} onChange={(event) => setName(event.target.value)} className="w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-stone-900" placeholder="Your name" />
						</label>
						<label className="space-y-2 text-sm text-stone-600">
					...
					<label className="block space-y-2 text-sm text-stone-600">
						...
						<textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={4} className="w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-stone-900" placeholder="What did you like about this product?" />
					</label>
					{error ? <p className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
					<p className="text-xs text-stone-500">Reviews are moderated and will appear publicly after admin approval.</p>
			...
			{successMessage ? <p className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{successMessage}</p> : null}

			<div className="mt-6 divide-y divide-stone-200 border-t border-stone-200">
				{initialReviews.length === 0 ? (
					<p className="py-6 text-sm text-stone-500">No reviews yet. Be the first to review this product.</p>
				) : (
					initialReviews.map((review) => (
						<div key={review.id} className="py-5">
							<div className="flex flex-wrap items-center justify-between gap-2">
								...
								<span className="text-xs text-stone-500">{formatDate(review.createdAt)}</span>
							</div>
							{review.title ? <p className="mt-2 text-sm font-medium text-stone-900">{review.title}</p> : null}
							<p className="mt-1 text-sm leading-6 text-stone-600">{review.comment}</p>
						</div>
					))
				)}
			</div>
